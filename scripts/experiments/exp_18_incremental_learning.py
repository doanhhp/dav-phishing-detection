...
def main():
    print("--- Phase 8: Incremental Learning (Domain Adaptation) ---")
    
    # 1. Load Pre-processed 2021 Training Data to train baseline
    ...
    # Train Initial Baseline Model on 2021 Data
    print("Training frozen 2021 Baseline XGBoost...")
    model = XGBClassifier(
        n_estimators=250, 
        learning_rate=0.05, 
        max_depth=8, 
    ...
    print(f"2026 Data Split: Stream={X_stream.shape[0]} samples, Holdout={X_test.shape[0]} samples.")
    
    # Initial Baseline Accuracy on 2026 Holdout Test Set
    ...
    for i in range(num_chunks + 1):
        ...
        # Incremental Update (xgb_model passes the existing tree structure)
        # We set n_estimators slightly higher each time so it can add a few new trees to correct errors
        ...
        # Evaluate on static holdout set
    ...
    plt.plot(samples_learned, accuracies, marker='o', linewidth=2, color='#2ecc71', markersize=6)
    plt.axhline(y=baseline_acc, color='#e74c3c', linestyle='--', label='Initial Zero-Day Collapse')
    
    plt.title('Domain Shift Adaptation: XGBoost Accuracy vs. Data Learned', fontsize=14, pad=15)
    plt.xlabel('Number of 2026 Domain Samples Learned', fontsize=12)
    plt.ylabel('Accuracy on 2026 Holdout Set', fontsize=12)
    plt.ylim(0, 1.0)
    plt.legend()
    
    out_dir = Path("docs/assets")
    out_dir.mkdir(parents=True, exist_ok=True)
    plt.savefig(out_dir / "incremental_learning.png", dpi=300, bbox_inches='tight')
    plt.close()
    
    print("\nExperiment Complete. Plot saved to docs/assets/incremental_learning.png")
