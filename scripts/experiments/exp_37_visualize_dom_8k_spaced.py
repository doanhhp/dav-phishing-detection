...
def plot_probabilistic_tree(G, title, filename, max_depth):
    ...
    # Scale canvas to STRICTLY max 8K resolution
    if max_depth <= 6:
    ...
    # Drastically reduced physical sizes to maximize spacing within the fixed 8K resolution
    node_sizes = [max(50, (freqs[node]**0.6) * node_multiplier) for node in G.nodes()]
    ...
    for u, v in G.edges():
        ...
        edge_widths.append(max(0.2, (weight**0.6) * 3))
    ...
    for node, (x, y) in pos.items():
        ...
        # Reduced font sizes significantly to ensure text does not bleed into neighboring nodes
        if max_depth <= 6:
            font_size = int(max(4, freq * 10))
        else:
            font_size = int(max(4, freq * 11))
    ...
    for f in ref_freqs:
        legend_elements.append(Line2D([0], [0], marker='o', color='w', 
                                      ...
                                      markersize=max(6, f * (10 if max_depth<=6 else 18)), 
...
def main():
    print("--- Phase 11: 4K/8K Optimized & Spaced DOM Tree Visualization ---")
    ...
    print("Plotting Phishing DOM Tree (4K Spaced)...")
    ...
    print("Plotting Benign DOM Tree (4K Spaced)...")
    ...
    print("Plotting Phishing DOM Tree (8K Spaced)...")
    ...
    print("Plotting Benign DOM Tree (8K Spaced)...")
    ...
    print("\nVisualizations Complete. Images are constrained to 4K/8K and spaced aggressively.")
