...
def plot_probabilistic_tree(G, title, filename, base_color):
    ...
    # Better balanced canvas size so nodes don't look tiny
    ...
    # Massively increased node sizes so they are highly visible
    node_sizes = [max(1000, freqs[node] * 20000) for node in G.nodes()]
    ...
    node_colors = []
    for node in G.nodes():
        alpha = max(0.1, freqs[node]) # Min opacity 0.1
    ...
    for u, v in G.edges():
        ...
        edge_widths.append(max(2.0, weight * 15))
    ...
    # Massively increased font sizes for labels
    for node, (x, y) in pos.items():
        freq = freqs[node]
        if freq > 0.03:
            font_weight = 'bold' if freq > 0.3 else 'normal'
            font_size = int(max(14, freq * 35)) # Font scales from 14 up to ~35
    ...
    plt.title(title, fontsize=42, pad=35)
    plt.axis('off')
...
def main():
    print("--- Phase 11: Perfect Scale Probabilistic DOM Tree Visualization ---")
    ...
    plot_probabilistic_tree(
        G_phish, 
        "Aggregated Phishing DOM Structure (Optimized Visibility)", 
        "probabilistic_dom_phishing_perfect.png", 
        "#b40426"
    )
    ...
    plot_probabilistic_tree(
        G_benign, 
        "Aggregated Legitimate DOM Structure (Optimized Visibility)", 
        "probabilistic_dom_benign_perfect.png", 
        "#3b4cc0"
    )
