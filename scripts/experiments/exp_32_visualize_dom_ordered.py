...
def hierarchy_pos(G, root, width=1., vert_gap=0.2, vert_loc=0, xcenter=0.5):
    ...
    def _hierarchy_pos(node, left, right, vert_loc):
        ...
        # Ensure consistent ordering so 'head' is on the left and 'body' is on the right
...
def build_probabilistic_tree(html_series, max_depth=8, min_freq=0.015):
    ...
    for xpath, count in xpath_counts.items():
        ...
        if freq >= min_freq:
            ...
            if len(parts) > 1:
                ...
                # Ensure parent exists, if not, add it so we don't break the tree
...
def plot_probabilistic_tree(G, title, filename, base_color):
    ...
    # Massive canvas to accommodate depth=8 and min_freq=0.015
    plt.figure(figsize=(75, 35))
    
    roots = [n for n, d in G.in_degree() if d == 0]
    root = roots[0] if roots else list(G.nodes())[0]
    
    # Increase vertical gap to accommodate more depth
    pos = hierarchy_pos(G, root=root, width=600.0, vert_gap=2.5, xcenter=0)
    ...
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', markerfacecolor=base_color, markersize=45, alpha=1.0, label='Very High Frequency (90%+)'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor=base_color, markersize=35, alpha=0.7, label='High Frequency (70%)'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor=base_color, markersize=25, alpha=0.5, label='Medium Frequency (50%)'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor=base_color, markersize=15, alpha=0.3, label='Low Frequency (30%)'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor=base_color, markersize=10, alpha=0.15, label='Rare Tags (<10%)')
    ]
    plt.legend(handles=legend_elements, loc='upper right', fontsize=26, title="Node Frequency Spectrum", title_fontsize=32)
            
    plt.title(title, fontsize=64, pad=50)
    plt.axis('off')
...
def main():
    print("--- Phase 11: Deep Ordered Probabilistic DOM Tree Visualization ---")
    ...
    G_phish = build_probabilistic_tree(phish_df['html'], max_depth=8, min_freq=0.015)
    
    print("Building Aggregated Benign DOM Tree...")
    G_benign = build_probabilistic_tree(benign_df['html'], max_depth=8, min_freq=0.015)
    
    print("Plotting Phishing DOM Tree...")
    plot_probabilistic_tree(
        G_phish, 
        "Aggregated Phishing DOM Structure (Deep & Ordered)", 
    ...
    plot_probabilistic_tree(
        G_benign, 
        "Aggregated Legitimate DOM Structure (Deep & Ordered)", 
