...
def main():
    print("--- Phase 9: Cross-Dataset Incremental Learning ---")
    ...
    print(f"\nPhreshPhish Split: Stream={X_stream.shape[0]} samples, Holdout={X_test.shape[0]} samples.")
    ...
    for i in range(num_chunks + 1):
        ...
        model.set_params(n_estimators=current_estimators + 5)
    ...
    plt.plot(samples_learned, accuracies, marker='o', linewidth=2, color='#3498db', markersize=4)
    plt.axhline(y=baseline_acc, color='#e74c3c', linestyle='--', label='Initial Cross-Dataset Collapse')
    
    plt.title('Cross-Dataset Adaptation: XGBoost Accuracy vs. Data Learned', fontsize=14, pad=15)
    ...
    plt.savefig(out_dir / "cross_dataset_incremental.png", dpi=300, bbox_inches='tight')
    plt.close()
    
    print("\nExperiment Complete. Plot saved to docs/assets/cross_dataset_incremental.png")
